...
- **Deliberately out of scope** (noted honestly): LinkedIn scraping (no public API);
  ML/NLP resume parsing — experience/education use deterministic regex/section heuristics
  and are flagged low-confidence; fuzzy cross-candidate identity beyond the 3-tier chain;
  a UI (the CLI is the surface).
